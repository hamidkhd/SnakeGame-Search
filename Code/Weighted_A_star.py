...
def weighted_A_star(start, map_size, heuristic_num, weight):
    frontier = list()
    explored = dict()
    ...
    heapq.heappush(frontier, ((weight * start.heuristic) + start.path_cost, start))
    explored[start.state] = (weight * start.heuristic) + start.path_cost
    ...
    while frontier:
        ...
        for i in directions:
            ...
            if _new_state not in explored or (weight * heuristic_value) + x.path_cost + 1 < explored[_new_state]: 
                heapq.heappush(frontier, ((weight * heuristic_value) +  x.path_cost + 1,
                    Node(x, _new_state, i, x.path_cost + 1, heuristic_value)))
                explored[_new_state] = (weight * heuristic_value) + x.path_cost + 1  
                separates_states += 1       
...
def main():
    ...
    for i in range(2):
        ...
        for i in range(len(input_files)):
            ...
            result, path_cost, states, separates_states = weighted_A_star(new_node, map_size, heuristic_num, weight)
