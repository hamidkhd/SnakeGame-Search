...
def DFS(start, map_size, depth):
    ...
    explored[start.state] = depth

    while frontier:
        x = frontier.pop()

        if x.path_cost == depth:
            continue

        if len(x.state.seeds) == 0:
            return True, x, states, separates_states
        ...
        for i in directions:
            ...
            states += 1
            if _new_state not in explored or x.path_cost + 1 < explored[_new_state]: 
                frontier.append(Node(x, _new_state, i, x.path_cost + 1))
                explored[_new_state] = x.path_cost + 1
                separates_states += 1

    return False, None, states, separates_states

def IDS(start, map_size):
    states = int(1)

    for i in itertools.count(start = 1):
        status, y, _states, separates_states = DFS(start, map_size, i)
        states += _states
        if status == False:
            continue
        elif len(y.state.seeds) == 0:
            result = list()
            search = y
            while search.parent != None:
                result.append(search.action)
                search = search.parent
            return result, y.path_cost, states, separates_states
...
def main():
    ...
    for i in range(len(input_files)):
        ...
        result, path_cost, states, separates_states = IDS(new_node, map_size)
